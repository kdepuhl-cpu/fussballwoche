...
export default async function VereinDetailPage({ params }: PageProps) {
  ...
  const club = getVereinBySlug(slug);

  if (!club) {
    notFound();
  }

  // Find related articles: match by vereinIds or by club name in tags
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div className="space-y-3">
                {club.kontakt.telefon && (
                  <a
                    href={`tel:${club.kontakt.telefon.replace(/\s/g, "")}`}
                ...
                {club.kontakt.email && (
                  <a
                    href={`mailto:${club.kontakt.email}`}
                    className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400 hover:text-forest-green dark:hover:text-green-400 transition-colors"
                  >
                    <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export async function generateStaticParams() {
  return getAllVereinSlugs().map((slug) => ({ slug }));
}
